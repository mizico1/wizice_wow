...
$(document).ready(function(){
    ...
    $('.menu > li').on("touchstart", function (e) {
        ...
        if (link.hasClass('selected')) {
            ...
            $('.menu > li').not(this).removeClass("selected");
            e.preventDefault();
...
function mobile_ver_menu() {
    $('#submenu h2 a').toggle(function() {
        $('#submenu').addClass('selected');
        $('#submenu h2 a').html('닫기');
        $('.submenu__wrap').animate({
            opacity : 1,
            marginLeft : 0
        }, 350);
    }, function() {
        $('#submenu').removeClass('selected');
        $('#submenu h2 a').html('메뉴열기');
        $('.submenu__wrap').animate({
            opacity : 0,
            marginLeft : -280
        }, 350);
...
function menuSizing() {
    var container = $('.lnb_wrap .menu');
    var elements = $('.lnb_wrap .menu > li');
    
    var menuWidth = $('.lnb_wrap .menu > li').length;   // total menu count 
    $('.lnb_wrap .nav-item ').css( 'width', (100/menuWidth) + '%');
}
...
function mainBbs2() {
    var oldIndex = 0;
    $('.bbs_list2 li').click(function() {//alert($('div').index(this));
...
function m_gnb() {
    $('.m_nav_open').click(function(){
        $('.m_lnb_wrap').addClass('selected');
        //$('.menu li:first-child').addClass('selected');
    });
    $('.m_nav_close').click(function(){
        $('.m_lnb_wrap').removeClass('selected');
    });
/*  $('.m_lnb_wrap  .menu >   li > a ').click(function(e){
            e.preventDefault();
            var thisIndex = $('.menu a').index(this);

            if ( document.body.clientWidth < 770 ) {
                $('.menu li').eq(thisIndex).find('a:eq(0)').unbind();
            }

        if ( $('.menu li').eq(thisIndex).hasClass('selected') ) {
               // location.href = $('.menu li').eq(thisIndex).find('a:eq(0)').attr('href');
        } else {
            $('.menu li').removeClass('selected');
            $('.menu li').eq(thisIndex).addClass('selected');
            return false;
        }
    });*/
}
